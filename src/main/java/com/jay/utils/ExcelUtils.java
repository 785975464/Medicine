...
public class ExcelUtils {
     ...
     * @throws IOException
     */
    public static void export(String excelName, String sheetName, String[] headers, String[] ds_titles, int[] ds_format, int[] widths, List<Medicine> data , HttpServletRequest request, HttpServletResponse response) throws IOException {
...
//        HSSFSheet sheet = wb.createSheet(StringUtils.isNotEmpty(sheetName)?sheetName:"excel");
        HSSFSheet sheet = wb.createSheet((sheetName!=null && !sheetName.equals("") )?sheetName:"excel");
        //创建表头，如果没有跳过
        int headerrow = 0;
        if(headers!=null){
...
//                }
                ...
                row.createCell(1).setCellValue(data.get(i).getLocationid());
                row.createCell(2).setCellValue(data.get(i).getName());
                row.createCell(3).setCellValue(data.get(i).getCas());
                row.createCell(4).setCellValue(data.get(i).getRemark());
                headerrow++;
            }
        }
